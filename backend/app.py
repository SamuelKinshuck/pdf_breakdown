...
def _pdf_path_for_file_id(file_id: str) -> str:
    ...
    if not pdf_path.exists():
        raise FileNotFoundError(f"PDF not found for file_id={file_id} at {pdf_path}")
    return str(pdf_path)
...
def _ensure_pdf_in_folder(original_path: Path, dest_dir: Path) -> Path:
    ...
    try:
        result = subprocess.run(
            ['soffice', '--headless', '--convert-to', 'pdf', '--outdir', str(dest_dir), str(original_path)],
        ...
        if not produced.exists():
            raise FileNotFoundError(f"LibreOffice reported success but PDF not found: {produced}")
...
def _compose_user_prompt(role: str, task: str, context: str, fmt: str, constraints: str) -> str:
...
def process_document():
    ...
    if not isinstance(selected_pages, list) or not selected_pages:
        return jsonify({'success': False, 'error': 'selected_pages must be a non-empty list'}), 400

    # Resolve canonical PDF for this upload
    try:
        pdf_path = _pdf_path_for_file_id(file_id)  # uploads/<file_id>/document.pdf
    except Exception as e:
        return jsonify({'success': False, 'error': f'Could not resolve PDF: {e}'}), 400
    ...
    if not rows:
        return jsonify(
            {'success' : False, 'message' : 'could not find selected pages'}
        )
